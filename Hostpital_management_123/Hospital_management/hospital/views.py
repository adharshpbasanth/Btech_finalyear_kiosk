...
from xhtml2pdf import pisa  # Ensure this library is installed
...
def download_pdf(request, patient_id):
    ...
    # Fetch discharge details (most recent entry if multiple exist)
    try:
        discharge_details = PatientDischargeDetails.objects.filter(patientId=patient.id).latest('id')
    except PatientDischargeDetails.DoesNotExist:
        return HttpResponse("Discharge details not found for the patient.", status=404)

    # Prepare context with Patient and Discharge Details
    context = {
        'patient': patient,
        'patientName': patient.get_name,
        'mobile': patient.mobile,
    ...
    template = get_template('hospital/patient_discharge.html')
    html = template.render(context)

    # Create a PDF response
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="Patient_{patient.id}_Invoice.pdf"'
